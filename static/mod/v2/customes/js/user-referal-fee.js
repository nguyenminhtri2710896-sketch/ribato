        ...
        var arrColumns = [
            ...
            },
            {
                ...
                mRender: function (data, type, row) {
                    ...
                    html += '<li data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Sửa"><button  data-params=\'{"query":{"id":' + row.id + '}}\' onclick="userReferalFee.detail(this)" class="btn btn-sm btn-soft-info  d-flex"  data-reload-datatable="#data-table-list"><i class="mdi mdi-pencil-outline"></i> sửa</button></li>';
                    html += '</ul>';
                    return html;
                }
            },
        ];
        base.getDataTableBasic("#data-table-user-referal-fee-list", arrColumns)
    }, detail: function (ts) {
        base.buttonAjax(urlAjaxUserReferalFeeDetail, ts,
            function (result) {
                if (result.error_code != 0) {
                    return toastr["error"](result.message)
                }

                var modal = $(".bs-modal-update-user-referal-fee");
                $.each(result.data.user_referal_fee, function (k, i) {
