...
@section('javascript')
    <!-- INSET JAVA SCRIPT -->
    <script src="{{ asset('backend/v1/customes/js/transaction.js') }}?v={{ config('app.asset_version') }}"></script>
...
@section('content')
                    ...
                    <div class="page-title-right ajax-select2">
                        <div class="btn-group  d-md-inline-block d-block d-grid mb-0  mt-1">
                            <button type="button" class="btn btn-lg-custom btn-success dropdown-toggle"
                                ...
                                {{ __('backend.create_qr') }}
                                <i class="mdi mdi-chevron-down"></i></button>
                            <div class="dropdown-menu action-dropdown-menu-mobile" style="">
                                <a class="dropdown-item  btn-create-qr-transaction" href="#"><i
                                        class="fas fa-plus-circle"></i> {{ __('backend.create_transaction_qr') }}</a>
                                <a class="dropdown-item  btn-create-qr-user" href="#"><i class="fas fa-plus-circle"></i>{{ __('backend.create_user_qr') }}</a>
                            </div>
                        </div>
                        <div class="btn-group  d-md-inline-block d-block d-grid mb-0 mt-1">
                                <button type="button"
                            ...
                            <div class="dropdown-menu dropdown-menu-md p-3 dropdown-menu-search">
                                <form id="table-filer" autocomplete="off">
                                    ...
                                    @if (auth()->user()->is_admin)
                                        <div class="mb-2">
                                            <label class="form-label" for="exampleDropdownFormRefCode">{{ __('backend.va_name') }}</label>
                                            <input type="text" name="bank_account_name" class="form-control"
                                                placeholder="{{ __('backend.va_name') }}">
                                        </div>
                                        <div class="mb-2">
                                        ...
                                        <div class="mb-2">
                                            <label class="form-label" for="exampleDropdownFormRefCode">{{ __('backend.content') }}</label>
    ...
    @include('backend.v1.transaction.modal.transaction-qrcode')
    @include('backend.v1.transaction.modal.user-qrcode')
