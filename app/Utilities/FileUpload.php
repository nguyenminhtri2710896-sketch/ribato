...
class FileUpload
{
    ...
    public $sizeLimit = 210485760;         // Max file upload size in bytes (default 10MB)
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null)
    {
        ...
        if (!file_exists($this->uploadDir)) {
            ...
            $this->setErrorMsg('Thư mục không tồn tại hoặc không có quyền ghi file');
            return false;
        }
        ...
        if (!empty($this->allowedExtensions)) {
            if (!$this->checkExtension($this->fileExtension, $this->allowedExtensions)) {
                $this->setErrorMsg('File không đúng định dạng');
                return false;
            }
        }
        $this->fileName = date('YmdH') . "_" . md5(time() . rand(1000000, 9999999)) . "." . $this->fileExtension;
    ...
    public function setSuccessResponse($arrSuccessResponse = [])
    {
        $this->arrSuccessResponse = $arrSuccessResponse;
    }

    public function getSuccessResponse()
    {
        return $this->arrSuccessResponse;
    }
}
